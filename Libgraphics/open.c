...
/* $Id: open.c,v 1.1 2002/10/09 15:12:26 peskine Exp $ */

// Bibliothèque stdlib ajoutée pour fct exit
#include <stdlib.h>
#include <fcntl.h>
#include <signal.h>
#include "libgraph.h"
...
void gr_open_graph(char* arg)
{
  ...
  if (gr_initialized) {
    ...
    /* Parse the argument */
    for (p = arg, q = display_name; *p != 0 && *p != ' '; p++)
  ...
  signal(EVENT_SIGNAL, &gr_sigio_handler);
  alarm(1);
#endif
...
int gr_sigio_signal(void)
{
  return EVENT_SIGNAL;
}

void gr_sigio_handler(int s)
{
  XEvent grevent;
